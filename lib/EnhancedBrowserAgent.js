...
class EnhancedBrowserAgent {
    constructor(options = {}) {
        ...
        this.config = {
            ...
            enableLogging: true, // Force enable logging for debugging
    ...
    async executeAutonomousTask(taskDescription, options = {}) {
        ...
        try {
            ...
            // Notify about session creation if callback provided
            if (options.sessionCallback && this.session) {
                const replayUrl = this.session.replayUrl || (this.session.id ? `https://app.browserbase.com/sessions/${this.session.id}` : null);
                options.sessionCallback({
    ...
    async observe() {
        ...
        try {
            ...
            this.log(`Page URL: ${observation.url}`, 'debug');
            this.log(`Page Title: ${observation.title}`, 'debug');
            
            // Check if page is loaded
            const readyState = await this.page.evaluate(() => document.readyState);
            this.log(`Page ready state: ${readyState}`, 'debug');
            
            // Take screenshot for visual analysis by Gemini Vision model
            try {
                ...
                if (Buffer.isBuffer(screenshotData)) {
                    observation.screenshot = screenshotData.toString('base64');
                    this.log(`Screenshot converted from Buffer to base64 string: ${observation.screenshot.length} characters`, 'debug');
                } else if (typeof screenshotData === 'string') {
                    observation.screenshot = screenshotData;
                    this.log(`Screenshot returned as base64 string: ${observation.screenshot.length} characters`, 'debug');
                } else {
                ...
                // Final validation
                if (!observation.screenshot || observation.screenshot.length < 100) {
                    this.log(`Warning: Screenshot invalid or too short: ${observation.screenshot ? observation.screenshot.length : 'null'} characters`, 'warning');
                    observation.screenshot = null;
                } else {
                    this.log(`Screenshot ready for Gemini Vision: ${observation.screenshot.length} characters`, 'success');
                }
            } catch (error) {
                this.log(`Screenshot capture failed: ${error.message}`, 'warning');
                observation.screenshot = null;
            }
    ...
    async evaluate(actionResult) {
        ...
        try {
            ...
            // Add screenshot if available
            if (currentObservation.screenshot) {
                parts.push({
                    inlineData: {
...
`;
    ...
    /**
     * Create a new browser session with improved error handling
     */
    async createSession() {
        try {
            this.log('Creating new browser session...', 'info');
            
            // First, try to clean up any existing sessions if we hit limits
            let sessionResponse;
            try {
                sessionResponse = await this.bb.createSession();
            } catch (error) {
                if (error.message.includes('concurrent sessions limit') || error.message.includes('429')) {
                    this.log('Concurrent session limit reached, cleaning up old sessions...', 'warning');
            ...
            if (!this.session || !this.session.connectUrl) {
                throw new Error('Invalid session response from Browserbase - missing connectUrl');
            }
            
            this.log(`Session created with ID: ${this.session.id}`, 'success');
            
            // Log the replay URL for viewing the automation
            const replayUrl = this.session.replayUrl || `https://app.browserbase.com/sessions/${this.session.id}`;
            this.log(`🎬 Watch automation live: ${replayUrl}`, 'info');
    ...
    /**
     * Clean up old sessions to free up concurrent session slots
     */
    async cleanupOldSessions() {
        try {
            this.log('Cleaning up old sessions...', 'info');
            
            // Get all sessions for this project
            const response = await fetch(`https://api.browserbase.com/v1/sessions?projectId=${this.projectId}`, {
                method: 'GET',
                headers: {
            ...
            // Find sessions that are RUNNING or have been running for a while
            const now = new Date();
            const oldSessions = sessions.filter(session => {
                if (session.status === 'RUNNING') {
                    const startedAt = new Date(session.startedAt);
                    const runningTime = now - startedAt;
                    // Clean up sessions running for more than 10 minutes
                    return runningTime > 10 * 60 * 1000;
                }
                return false;
            });
            
            this.log(`Found ${oldSessions.length} old sessions to clean up`, 'info');
            
            // Note: Browserbase doesn't have a public DELETE endpoint, 
            // but sessions should auto-expire based on their expiresAt time
            // We'll log the sessions that should be cleaned up
            for (const session of oldSessions) {
                this.log(`Old session found: ${session.id} (started: ${session.startedAt})`, 'warning');
            }
